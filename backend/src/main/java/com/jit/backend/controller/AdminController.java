...
public class AdminController {
    ...
    @Operation(summary = "회원조회", description = "Admin의 모든 회원 조회 기능입니다. " +
            "<br>Role이 ADMIN인 회원만 접속할 수 있습니다.")
    @GetMapping("/users")
    public @ResponseBody
    ...
    @Operation(summary = "Factory Name에 해당하는 회원조회", description = "Admin의 모든 회원 조회 기능입니다. " +
            "<br>Role이 ADMIN인 회원만 접속할 수 있습니다." +
            "<br>파라미터의 factoryName이 User의 FactoryName과 같은 회원을 조회합니다.")
    @GetMapping("/users/{factoryName}")
    public @ResponseBody
    ResponseEntity findMemberForFactoryName(
            @Parameter(description = "파라미터는 Factory Name을 입력합니다. <br>ex) CarFactory")
            @PathVariable String factoryName) {
        List<User> userList;
        try {
            userList = userService.findUserByFactoryName(factoryName);
        } catch (IllegalStateException e) {
            return new ResponseEntity(e.getMessage(), HttpStatus.BAD_REQUEST);
        }
        return new ResponseEntity(userList, HttpStatus.OK);
    ...
    @Operation(summary = "Admin 회원가입", description = "Admin의 회원가입 기능입니다. " +
            "<br>여기서 회원가입을 하면 Role이 ADMIN인 회원이 생성됩니다.")
    @PostMapping("/signup")
    ...
    @Operation(summary = "User 권한변경", description = "Admin이 User의 권한을 변경하는 기능입니다. " +
            "<br>Role이 ADMIN인 회원만 접속할 수 있습니다. " +
            "<br>\"role\": \"role종류\" 만 작성해도 변경이 가능합니다.")
